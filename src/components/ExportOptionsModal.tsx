...
export default function ExportOptionsModal({
  ...
  const handleExport = () => {
    let dataToExport = filteredData
      .filter(item => {
        // Exclure les patients non payés
        const amount = parseFloat((item.amount || '0').replace(',', '.'));
        return amount > 0;
      })
      .map(item => {
        const patientName = item.patientDetails ? 
          formatters.patientName(item.patientDetails.nom, item.patientDetails.prenom) :
          item.nom && item.prenom ? 
            formatters.patientName(item.nom, item.prenom) :
            'Patient non spécifié';

        return {
          numeroPatient: formatters.patientNumber(item.patientNumber),
    ...
    if (!isNaN(limit) && limit > 0) {
      if (patientsAvecMutuelle.length >= limit) {
        // Si le nombre de patients avec mutuelle dépasse la limite,
        // on exporte uniquement les patients avec mutuelle
        dataToExport = patientsAvecMutuelle;
      } else {
        // Sinon, on prend tous les patients avec mutuelle
        // et on complète avec des patients sans mutuelle jusqu'à la limite
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-1 text-xs text-gray-500 space-y-1">
                <p>Note: Si le nombre de patients avec mutuelle dépasse la limite, seuls les patients avec mutuelle seront exportés.</p>
                <p>Les patients non payés ne seront pas inclus dans l'exportation.</p>
              </div>
            </div>
